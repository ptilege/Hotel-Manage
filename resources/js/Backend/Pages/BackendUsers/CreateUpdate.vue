          ...
          <div class="card-body">
            ...
            <form id="formAccountSettings" @submit.prevent="submit">
              <div class="row">
                ...
                <div class="mb-3 col-md-6">
                  <label for="property" class="form-label">Property <small>(Select property if user create for property)</small> </label>
                  <select
                    class="select2 form-control form-select"
                    id="property"
                    aria-label="Default select example"
                ...
                <div class="mb-3 col-md-6">
                  <label for="profile_image" class="form-label me-3"
                    >Profile Image</label
                  >
                  <br />
                  <FileInputComponent
...
export default {
  ...
  mounted() {
    ...
    if (this.backendUser) {
      ...
      this.form.property =
        this.backendUser.properties.length > 0 ? this.backendUser.properties.map((pr) => pr.id) : [];
    }
    $(".card-body").on("change", "#role", function (evt) {
      self.form.role = $(evt.target).val();
    });
    $(".card-body").on("change", "#property", function (evt) {
      self.form.property = $(evt.target).val();
    });
  },
  computed: {
    profileImage() {
