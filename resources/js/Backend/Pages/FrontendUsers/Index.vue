        ...
        <div class="card mb-4">
          <div class="card-header pb-0">
            ...
            <div class="d-flex">
              <Link
                ...
                :href="route('settings.front-user.create')"
                v-if="$root.hasPermission('frontend-user.create')"
              >
          ...
          <div class="card-body">
            <data-table
              ref="datatable"
              :id="'mytable'"
              :url="route('settings.front-user.getdata')"
              :columns="columns"
...
export default {
  ...
  mounted() {
    ...
    $("#mytable tbody").on("click", "tr .action_edit", (evt) => {
      ...
      this.$inertia.visit(route("settings.front-user.edit", id));
    });
  ...
  methods: {
    ...
    updateStatus(id, status) {
      ...
      this.form.put(route("settings.front-user.change.status"), {
        preserveScroll: true,
        onSuccess: () => {
          this.form.reset();
          this.reloadTable();
    ...
    deleteSelectedItems() {
      this.form
        ...
        .post(route("settings.front-user.delete"), {
          onSuccess: () => {
            this.resetForm();
            this.reloadTable();
            this.$root.showMessage(
