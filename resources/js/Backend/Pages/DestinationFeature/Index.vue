      ...
      <div class="col-md-12">
        <div class="card mb-4">
          <div class="card-header pb-0">
            <h5>Destination Features</h5>
            <p>Manage Destination Features.</p>
            <div class="d-flex">
              <Link
                class="btn btn-primary btn-sm"
                :href="route('destination.features.create')"
                v-if="$root.hasPermission('destination-feature.create')"
              >
          ...
          <div class="card-body">
            <data-table
              ref="datatable"
              :id="'mytable'"
              :url="route('destination.features.getdata')"
              :columns="columns"
...
export default {
  ...
  mounted() {
    ...
    $("#mytable tbody").on("click", "tr .action_edit", (evt) => {
      ...
      this.$inertia.visit(route("destination.features.edit", id));
    });
  ...
  methods: {
    ...
    updateStatus(id, status) {
      ...
      this.form.put(route("destination.features.status"), {
        preserveScroll: true,
        onSuccess: () => {
          this.form.reset();
          this.reloadTable();
    ...
    deleteSelectedItems() {
      this.form
        ...
        .post(route("destination.features.delete"), {
          onSuccess: () => {
            this.resetForm();
            this.reloadTable();
            this.$root.showMessage(
              "success",
              '<span class="text-success">Success</span><br/>',
              "A Record Was Deleted Successfully! "
            );
          },
          onError: () => {
            this.$root.showMessage(
