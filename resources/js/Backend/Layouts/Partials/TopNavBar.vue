<template>
  ...
  >
    ...
    <ul class="navbar-nav flex-row align-items-center me-auto">
      <li v-if="$page.props.backend_property">
        <img :src="$page.props.backend_property ? $page.props.backend_property.image : ''" alt="" style="width:45px;height:45px;object-fit:cover;">
      </li>
      <li class="ms-2">
        <p class="m-0" v-if="$page.props.backend_property">Selected Property <small class="text-danger">(Select "Super Admin" from the dropdown for roomista configuration)</small>:</p>
      <h3 class="m-0">{{$page.props.backend_property ? $page.props.backend_property.name : "Roomista Configuration"}}</h3>
      </li>
    </ul>
...
export default {
  ...
  mounted() {
    var self = this;
    if(this.$page.props.backend_property) {
      this.propertySelect.id = this.$page.props.backend_property.id
    }

    $("#property_select").select2();
    $("#property_select").on("change", function (evt) {
      self.propertySelect.id = $(evt.target).val();
      self.propertySelect.post(route("properties.set.backend"), {
        onSuccess: () => {
          Inertia.visit(route('dashboard'));
          self.$root.showMessage(
            "success",
            '<span class="text-success">Success</span><br/>',
            "Property Change Successfully! "
          );
        },
      });
    });
